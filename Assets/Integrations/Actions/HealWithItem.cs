...
{
    public static class HealWithItem
    {
        public static HealActionPayload MakePayload(ItemInstance source, GameObject target, int amount)
        {
            var basePayload = new ActionActivationPayload(new ActionBase(nameof(HealWithItem)), source, target);
        ...
        {
            ActionImplementation implementation = new(nameof(HealWithItem), OnActivation, PayloadMake);
